...
export default function Result() {
  ...
  const MovieCard = ({ m }: { m: Movie }) => (
    <motion.div className="featured-movie" whileHover={{ y: -4, boxShadow: "0 20px 48px rgba(0,0,0,0.75)" }} transition={{ duration: 0.25, ease: "easeOut" }}>
  ...
  const SecondaryMovieCard = ({ m, index }: { m: Movie; index: number }) => {
    ...
    return (
      <motion.div
        ...
        whileHover={{ y: -5 }}
      >
        <div className="seconda｀ry-poster-wrap">
  ...
  return (
    <div className="result-page">
          ...
          <div className="card-inner">
            <div className="card-col-left">
              ...
              <div className="result-side">
                <h3 className="result-heading">{isTagMode ? `根據你的心情：${tagNames.map((t) => `#${t}`).join(" ")}` : "今晚適合怎麼看電影？"}</h3>
            ...
            <div className="card-col-right">
              ...
              {!loading && !error && isTagMode && movies.length > 0 && <PrimaryMovieCard m={movies[0]} />}
      ...
      {!loading && !error && isTagMode && movies.length > 1 && (
